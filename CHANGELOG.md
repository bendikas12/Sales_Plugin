## 0.6.4 - 2026-05-13
- Fixed: sales-dashboard-artifact n8n reliability — wrapped every n8n MCP call (`execute_workflow`, `get_execution` poll, `get_execution` data) in a `Promise.race` timeout (30s / 20s / 45s) so hung calls surface as errors instead of leaving cards stuck on "Waiting for n8n…". Replaced exponential-backoff poll with a 6s prewait + fixed 4s interval (90s deadline) and explicit `PENDING` / `TERMINAL` status sets covering `started`/`waiting`/`pending` alongside `running`/`new`. Added a 4-attempt retry loop on the `includeData: true` fetch (with 2s post-success pause + 3s between attempts) to handle runData lag. Captured the last `get_execution` response shape in poll-timeout errors for remote diagnostics. Overdue Deals and Spend Gap now launch in a parallel IIFE with a 250ms launch stagger to avoid lockstep MCP-server contention while still loading concurrently with the rest of the dashboard.

## 0.6.3 - 2026-05-13
- Changed: unread-inbox-artifact AI draft prompt now includes a greeting line addressing the sender by first name (falls back to "Hi,") instead of stripping it. Adjustment prompt preserves the existing greeting unless the instruction explicitly asks to change it.
- Changed: unread-inbox-artifact "Sent" badge renamed to "Draft Created" to match the Draft Created tab and reflect that the action creates a Gmail draft (not a sent message).
...
## 0.6.0 - 2026-05-13
- Added: sales-dashboard-artifact skill — installs a persistent Cowork artifact (id `sales-dashboard`) that self-refreshes every metric the static `sales-dashboard` skill produces (HubSpot calls/emails/tasks, pipeline open-deals + expected volume, by-stage bar chart, 30-day funnel throughput for Demo Scheduled / Submitted to credit / Account activated, Gmail unread, today's meetings + customer-facing count, and the n8n-fed Overdue Deals and Spend Gap tables). All fetches run via `window.cowork.callMcpTool` inside the artifact; the Reload button re-runs them in parallel with per-card skeleton states. Skill resolves the rep's identity, HubSpot owner ID, and the six MCP tool names (HubSpot `search_crm_objects` + `search_owners`, Gmail `search_threads`, Calendar `list_events`, n8n `execute_workflow` + `get_execution`) at install time and substitutes them into the template via `{{REP_NAME}}`, `{{REP_EMAIL}}`, `{{HUBSPOT_OWNER_ID}}`, `{{HS_SEARCH_CRM_TOOL}}`, `{{HS_SEARCH_OWNERS_TOOL}}`, `{{GMAIL_SEARCH_TOOL}}`, `{{CAL_LIST_EVENTS_TOOL}}`, `{{N8N_EXECUTE_TOOL}}`, `{{N8N_GET_EXECUTION_TOOL}}`. Sales pipeline ID, excluded stage IDs, and the three stage-entered timestamp property names are hardcoded in the template (sourced from `references/hubspot-glossary.md`). Template lives at `skills/sales-dashboard-artifact/references/artifact-template.html`. n8n tools are optional — if absent the two n8n tables render `N/A` while the rest of the dashboard works.

## 0.5.0 - 2026-05-13
- Added: sales-dashboard renders an "Overdue Deals in Pipeline" table below the Pipeline-by-stage chart and above the Spend Gap table, fed by n8n workflow `LtpP0jfhfNzdLU6m` (production mode) scoped to the invoker's email via `chatInput`. Polls `get_execution` until `status != "running"`, fetches `includeData: true, nodeNames: ["Get row(s) in sheet"]`, sorts rows by "Days in pipeline" descending (falls back to "Days in current stage") and renders the top 5 with five columns: Deal name, Deal stage, Days in current stage, Total Addressable Spend, Expected Monthly Transaction Volume. Rows transformed programmatically into `{{OVERDUE_DEALS_ROWS_HTML}}` with case-insensitive header matching. Chat summary gains a matching top-5 block. Renders `N/A` if the n8n MCP is unavailable or the workflow doesn't finish with status `success`.
