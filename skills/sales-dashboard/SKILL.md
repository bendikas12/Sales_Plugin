...
1. Execute workflow `WpHzZ6nsAERzp2H5`, mode `production`, input `{ "type": "chat", "chatInput": "", "email": "<REP.email>" }` — substitute `<REP.email>` with the invoker's email resolved in Step 1 (e.g. `"email": "jane.doe@getpliant.com"`). **Never hardcode an email.** The workflow is purpose-built for spend-gap rows; `chatInput` stays empty (the field is required by the chat-trigger shape, but its content is ignored).
