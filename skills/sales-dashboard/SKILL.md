...
4. Match sheet headers case-insensitively after stripping non-alphanumerics so minor header renames don't break the table. HTML-escape cell values. **`Max Utilization` is a 0–1 ratio — multiply by 100 and append `%` to display as a percentage (e.g. `0.24` → `24%`, `0` → `0%`).** Format `Spending Gap` and `Hubspot Exp Monthly Trx Vol` as `€` with thousand separators. Empty cell → `N/A`.
