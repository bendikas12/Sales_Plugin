...
1. Execute workflow `WpHzZ6nsAERzp2H5`, mode `production`, input `{ "type": "chat", "chatInput": "biggest spend gap", "email": "<REP.email>" }` — substitute `<REP.email>` with the invoker's email resolved in Step 1 (e.g. `"email": "jane.doe@getpliant.com"`). **Never hardcode an email.** The `chatInput` string is the literal trigger the workflow's chat node routes on to call the Google Sheet tool — don't paraphrase it or the workflow returns empty.
