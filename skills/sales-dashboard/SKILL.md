...
1. Execute workflow `WpHzZ6nsAERzp2H5`, mode `production`, input `{ "type": "chat", "chatInput": "<REP.email>" }` — substitute `<REP.email>` with the invoker's email resolved in Step 1 (e.g. `"email": "jane.doe@getpliant.com"`). **Never hardcode an email.** The workflow is purpose-built for spend-gap rows; `chatInput` stays empty (the field is required by the chat-trigger shape, but its content is ignored).
2. Poll `get_execution` with `includeData: false`, backoff 2→4→8s capped at 30s, ceiling ~2 min, until `status != "running"`.
3. Fetch `get_execution` with `includeData: true, nodeNames: ["Get row(s) in sheet"]`. Rows = that node's output items mapped to their `json` payload.
4. Match sheet headers case-insensitively after stripping non-alphanumerics so minor header renames don't break the table. HTML-escape cell values. **`Max Util Exp Vol 0 365` is a 0–1 ratio — multiply by 100 and append `%` to display as a percentage (e.g. `0.24` → `24%`, `0` → `0%`).** Format `Spending Gap` and `Hubspot Exp Monthly Trx Vol` as `€` with thousand separators. Empty cell → `N/A`.
